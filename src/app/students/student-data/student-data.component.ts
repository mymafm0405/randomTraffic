...
export class StudentDataComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.students = this.studentService.students.filter(
      (student) => student.groupId === this.currentGroupId
    );
    console.log(this.currentGroupId);
    console.log(this.students);
    //
    this.groupService.groupIdChanged.subscribe((updatedGroupId: number) => {
    ...
    this.studentService.studentsChanged.subscribe(
      (updatedStudents: Student[]) => {
        this.students = updatedStudents.filter(
          (student) => student.groupId === this.currentGroupId
        );
        console.log(this.students);
      }
  ...
  onRemove(studentId: number) {
    const currentStudentIndex = this.studentService.students.findIndex(
  ...
  onEdit(studentId: number, studentIndex: number) {
    ...
    if (
      this.currentStudentIndex === undefined ||
  ...
  onSubmit(editForm: FormGroup) {
    ...
    this.studentService.students.find(
      (stu) => stu.id === this.currentStudentEdit.id
    ).name = editForm.value.studentName;
    this.studentFireId = this.studentService.studentsArrayWithFireId.find(
      (stu) => stu.id === this.currentStudentEdit.id
    ).fireId;
    if (this.studentFireId) {
      this.http
        .patch(
          "https://randomtraffic-default-rtdb.firebaseio.com/students/" +
            this.studentFireId +
            ".json",
          {
            name: editForm.value.studentName,
        ...
        .pipe(
          finalize(() => {
            fileRef.getDownloadURL().subscribe((url) => {
              ...
              this.studentService.studentsChanged.next(
  ...
  onPreview(event: any) {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e: any) =>
        (this.currentStudentEdit.imageUrl = e.target.result);
